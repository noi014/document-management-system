...
const pool = mysql.createPool({
  host: process.env.DB_HOST || 'gateway01.us-west-2.prod.aws.tidbcloud.com',
  port: parseInt(process.env.DB_PORT || '4000', 10), // ตรวจสอบให้แน่ใจว่า Port 4000 ยังอยู่
  user: process.env.DB_USER || '2wmePjWfByh6Xb3.root',
  password: process.env.DB_PASSWORD || 'cB5f1WA9MZJJTQ4E',
  database: process.env.DB_NAME || 'document_system',
  waitForConnections: true,
  ...
  ssl: { // <-- เพิ่มส่วนนี้เข้ามา
    rejectUnauthorized: true, // บังคับใช้ SSL และตรวจสอบ Server Certificate
  }
});
...
export async function getConnection(): Promise<PoolConnection> {
  try {
    ...
    // ไม่มี connection.release() ที่นี่ เพราะ pool.getConnection() จะจัดการเอง
    // คุณจะ release connection ในส่วนที่เรียกใช้ getConnection() เช่นใน route.ts
  }
}
...
export async function testDbConnection() {
  ...
  try {
    ...
    // สามารถทำ query ง่ายๆ เช่น SELECT 1; เพื่อทดสอบ
    await connection.query('SELECT 1;');
  } catch (error) {
    console.error('Failed to connect to the database:', error);
    throw error;
  } finally {
    if (connection) connection.release(); // อย่าลืม release connection ที่ได้มาจากการทดสอบ
  }
}